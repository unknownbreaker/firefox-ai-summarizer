...
- **Hiding a provider's rail does not necessarily reclaim its space — check whether the rail is in flow** — Claude's rail is `position: absolute`, so it never occupied layout space at all; `display: none` on it changes nothing about the conversation's width. The gutter is held open by a `padding-left: 288px` on `div.dframe-content-inner` (280px rail + 8px gap) that is indifferent to the rail's visibility, so `nav-hider.js` zeroes that padding too. Gemini (`position: relative`) and ChatGPT (`position: static`) are both in normal flow, so hiding alone collapses them correctly and they need no such rule. Measured live 2026-09-22; this was the v0.7.0 bug where the rail vanished but the conversation stayed offset.
...
| Hide/show the provider's own nav rail | `content/nav-hider.js` (selectors live here, NOT in providers.js — see its File Map row). Rail selectors verified live for all three (ChatGPT + Gemini 2026-09-19, Claude 2026-09-22) |
...
| `content/injector.js` | 726 | Runs on LLM pages in sidebar (inert in regular tabs — no `_t` marker). Receives prompts, attaches article (file input or page-realm drag-drop), fills input, clicks submit |
| `content/nav-hider.js` | 194 | Hides the provider's own conversation rail (and, for Claude, zeroes the padding that reserves its space) so it doesn't eat the narrow sidebar. Runs at `document_start` (injector.js is `document_idle` — CSS applied that late would flash the rail, then yank it). Gated on the same `_t` marker, so regular tabs on LLM domains are untouched. CSS is keyed by **hostname**, not provider id: it must resolve synchronously before any storage read, and the page's host is ground truth. Hides Gemini's rail rather than removing it — `newChatSelector` lives inside |
...
| `providers/providers.js` | 114 | Provider config (Gemini/Claude/ChatGPT/custom). Load/save from `storage.sync`, merge overrides |
...
- `test/injector-newchat.test.html` — 11 pass. Covers `startNewChat`'s three outcomes (clicked / already-fresh / stale-selectors-warn), that the Gemini logo anchor is never clicked, and that the freshness check does not block the injection flow
- `test/nav-hider.test.html` — 12 pass. Includes the regression guard that Gemini's "New chat" control stays in the DOM and clickable while its rail is hidden

When serving these locally, **cache-bust the URL** (`?v=2`) or the browser will happily re-run a stale copy and report the old result. **That is not enough on its own:** a cache-busted page still loads a plain `<script src="../content/foo.js">` from cache, so the suite grades a STALE copy of the code under test — which produces false FAILs and, worse, false PASSes. `nav-hider.test.html` therefore injects its script with a `?cb=<timestamp>` and runs the assertions from its `onload`; copy that pattern in new suites. Both failure modes have already bitten here.
...
- **Gemini**: input=`div.ql-editor[contenteditable='true']` (Quill editor), submit=`button[aria-label='Send message']`, fallbacks=[`button[aria-label*='Send']`, `button[mat-icon-button][aria-label*='Send']`], file=`input[type='file']`, newChat=`button[aria-label='New chat' i]`, newChatFallbacks=[`bard-sidenav a[aria-label='New chat' i]`]
...
`newChatFallbacks` (provider field, default none): alternates tried in order when `newChatSelector` matches nothing, mirroring `submitFallbacks`. **A fallback MUST be scoped to the conversation rail.** Gemini labels its *logo link* "New chat" as well, and that one is an `href="/"` navigation — clicking it reloads the injector and loses the pending prompt. Gemini's fallback is therefore `bard-sidenav a[aria-label='New chat' i]`.

### The 2026-09-22 investigation (read before touching `newChatSelector`)

**Established by live DOM probe** of `gemini.google.com/app`:
- `button[aria-label='New chat' i]` matched **zero** elements. The only "New chat" controls were two anchors: the sidenav one (`href="/app"`) and the logo (`href="/"`).
- The sidenav anchor's `aria-disabled` flips `"true"` → `"false"` the moment a conversation exists. **That is a usable freshness signal**, and `startNewChat` now verifies against it instead of assuming the click worked.
- Clicking the sidenav anchor does **not** reload the document (a `window` sentinel survived). So the CLAUDE.md warning about anchors applies to the *logo* anchor, not this one.

**RESOLVED 2026-09-22 (user-confirmed): `button[aria-label='New chat' i]` is still valid signed in. The primary selector is NOT broken — do not "fix" it.** The zero-match above was an artifact of the signed-OUT probe environment, nothing more.

**Do not re-run that probe expecting a different answer.** The chrome-devtools MCP spawns its own isolated, signed-out profile (`--user-data-dir=.../chrome-devtools-mcp/chrome-profile --disable-extensions`), and since Chrome 136 `--remote-debugging-port` is refused on the default profile directory — so the signed-in profile is unreachable by automation, full stop. That same isolated profile is why claude.ai returns Cloudflare challenges to these probes. **Signed-out Gemini is not a valid stand-in:** it renders a stripped sidenav and refuses to open a second conversation, so conclusions about the signed-in DOM drawn there are worthless. Ask the user instead — it takes them seconds.

**What the episode left behind that IS worth keeping:** `startNewChat` used to collapse "already fresh" and "control not found" into one silent early return, so if the selector ever *does* rot, every Gemini summary would append to the restored conversation with no trace. It now separates the three outcomes and `console.warn`s when nothing matches. The post-click freshness check is deliberately **not awaited** — the `aria-disabled` freshness signal was only observed on the anchor, so awaiting it could tax every summarize by the full verify budget on a provider that signals freshness differently.
