...
| `content/injector.js` | 721 | Runs on LLM pages in sidebar (inert in regular tabs — no `_t` marker). Receives prompts, attaches article (file input or page-realm drag-drop), fills input, clicks submit |
...
- `test/injector-newchat.test.html` — 10 pass. Covers `startNewChat`'s three outcomes (clicked / already-fresh / stale-selectors-warn) and that the Gemini logo anchor is never clicked
...
**Still NOT established, and it needs a signed-in session:** whether signed-in Gemini renders the `<button>` variant. The probe ran signed OUT — the chrome-devtools MCP spawns its own isolated profile (`--user-data-dir=.../chrome-devtools-mcp/chrome-profile`), and since Chrome 136 `--remote-debugging-port` is refused on the default profile dir, so the signed-in profile is not reachable by automation. Signed-out Gemini also refuses to open a second conversation, so the anchor fallback could not be exercised end-to-end either.

**Why this is no longer urgent:** `startNewChat` used to collapse "already fresh" and "control not found" into one silent early return, so a stale selector broke every Gemini summary invisibly. It now separates them and `console.warn`s when nothing matches at all. **To settle the open question, run one Gemini summarize and check the console:** a warning naming `newChatSelector` means the selectors are stale; silence means one of them matched.
